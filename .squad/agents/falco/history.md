...
- **Build system:** CMake + Ninja generator. Shell-agnostic command: `cmake -S . -B build -G Ninja` then `cmake --build build`.
...
- **Watchdog hazard:** Firmware enables 100 ms watchdog, calls `watchdog_update()` once at startup (line 157), then loops with 1000 ms sleep—device WILL reboot unless watchdog is fed in loop or disabled.
...
## Updates (Instruction Patch)
- Replaced build command with shell-agnostic sequence: `cmake -S . -B build -G Ninja` then `cmake --build build`.
- Clarified that there is no test suite, linter, or single-test command.
- Fixed watchdog documentation to accurately state that firmware will reboot (100 ms timeout, 1000 ms loop sleep, no feed in loop).
- Corrected UART reference in validation (UART1, not UART0 by default in this firmware).
